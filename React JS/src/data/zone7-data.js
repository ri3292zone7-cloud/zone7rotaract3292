...
/* zrrs/leadership are no longer cached (data is small and photo uploads
   ...
   a pre-existing browser tab may still hold. */
try{ localStorage.removeItem("z7c_zrrs"); localStorage.removeItem("z7c_leadership"); }catch(e){}
...
/* Resolve a stored image reference into an <img>-ready src. Stored values
   ...
   "media/team/Binaya.jpg" (seeded ZRR/leadership photos that ship with the app
   under /team/*). Only the bare paths need a leading slash. */
function zone7Photo(u){
  if(!u) return '';
  if(/^(data:)/i.test(u) || /^https?:\/\//i.test(u) || u.startsWith('/')) return u;
  return '/' + u;
}
...
const ZONE7_DB = {
  ...
  _fromRow(row){
    return {
      ...
      summary: row.summary,
      body: row.body,
      cover: zone7Photo(row.cover),
      gallery: (row.gallery || []).map(zone7Photo),
  ...
  async getClubProfile(clubSlug){
    try{
      ...
      const row = rows[0];
      if(row && Array.isArray(row.board)){
        row.board = row.board.map((m) => ({ ...m, photo: zone7Photo(m.photo) }));
      }
      return row;
    } catch(e){
  ...
  async getZRRs(){
    ...
    try{
      ...
      if(!res.ok) throw new Error("Fetch failed: " + res.status);
      const rows = await res.json();
      if(rows.length){ this._zrrCache = rows; return newestFirst(rows).map((r) => ({ ...r, photo: zone7Photo(r.photo) })); }
      ...
      return newestFirst(this._zrrFallback).map((r) => ({ ...r, photo: zone7Photo(r.photo) }));
    } catch(e){
      console.error("ZONE7_DB.getZRRs error", e);
      return newestFirst(this._zrrCache || this._zrrFallback).map((r) => ({ ...r, photo: zone7Photo(r.photo) }));
    }
  },
  ...
  async getLeadership(){
    try{
      ...
      if(!res.ok) throw new Error("Fetch failed: " + res.status);
      const rows = await res.json();
      if(rows.length){ this._leadershipCache = rows; return rows.map((r) => ({ ...r, photo: zone7Photo(r.photo) })); }
      // Seed fallback
      for(const l of this._leadershipFallback){ await this.saveLeader({...l}); }
      this._leadershipCache = this._leadershipFallback;
      return this._leadershipFallback.map((r) => ({ ...r, photo: zone7Photo(r.photo) }));
    } catch(e){
      console.error("ZONE7_DB.getLeadership error", e);
      return (this._leadershipCache || this._leadershipFallback).map((r) => ({ ...r, photo: zone7Photo(r.photo) }));
    }
  },
...
/* ES module exports (React port) */
export { SUPABASE_URL, SUPABASE_ANON_KEY, CLUB_DIRECTORY, CLUB_LETTERHEAD, CLUB_CREDENTIALS, ZONAL_PASSWORD, UNIVERSITY_CLUBS, BAROMETER_THRESHOLDS, zone7BarometerCategory, BAROMETER_GROUPS, zone7BarometerGroup, BAROMETER_COMMUNITY, BAROMETER_UNIVERSITY, zone7GetBarometer, ROTARY_FOCUS_AREAS, zone7InRY2627, zone7ItemCode, zone7AutoCheck, ZONE7_DB, zone7UploadImage, zone7ReadImage, zone7ReadFile, zone7Esc, zone7PrefersClub, zone7Slugify, CLUB_PROFILES_URL, BAROMETER_URL, LEADERSHIP_URL, GUIDES_URL, ZRRS_URL, GUEST_URL, APP_URL, REST_URL, EVENTS_URL, REST_HEADERS, STORAGE_BUCKET };
